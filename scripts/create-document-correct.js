// Script para crear documento con estructura correcta
// Ejecuta: node scripts/create-document-correct.js
...
async function createDocumentCorrect() {
    console.log('📄 CREANDO DOCUMENTO CON ESTRUCTURA CORRECTA\n');
    ...
    try {
        ...
        const testImagePath = path.join(__dirname, 'test-correct-document.png');

        fs.writeFileSync(testImagePath, Buffer.from(testImageData, 'base64'));
        console.log('   ✅ Imagen de prueba creada');

        // 3. Crear documento en la base de datos con estructura correcta
        ...
        const { data: document, error: docError } = await supabase
            ...
            .insert({
                tenant_id: 1,
                filename: 'test_correct_document.png',
                original_filename: 'test_correct_document.png',
                file_size: testImageData.length * 0.75,
                mime_type: 'image/png',
        ...
        const fileName = `whatsapp/${document.id}/test_correct_document.png`;
        ...
        if (finalDoc) {
            console.log('   ✅ Documento final:', {
                ...
                processing_method: finalDoc.processing_method
            });
        }
