...
function createSupabaseClient(useServiceRole = false) {
  ...
  return createClient(url, key, {
    ...
    global: {
      headers: {
        'x-client-info': 'delete-all-clients-api'
      }
    }
  })
}
...
export async function DELETE(request: NextRequest) {
  try {
    ...
    if (error) {
      ...
      return NextResponse.json({ error: 'Erro ao eliminar clientes' }, { status: 500 })
    }
    ...
    return NextResponse.json({ 
      success: true, 
      message: 'Todos os clientes foram eliminados com sucesso' 
    })
    
  } catch (error) {
    console.error('❌ Error in delete all clients:', error)
