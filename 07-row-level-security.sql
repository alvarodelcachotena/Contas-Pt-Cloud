-- Row-Level Security (RLS) Policies for Contas-PT Cloud
...
-- RAG query log policies
CREATE POLICY "Users can view their own query logs" ON rag_query_log
    FOR SELECT USING (
        user_id = auth.uid() OR 
        (user_id IS NULL AND has_tenant_access(tenant_id))
    );
...
DO $$
BEGIN
    RAISE NOTICE '✅ Row-Level Security policies configured successfully';
END $$;
